...
#Graph winners
JT_portfolio %>% ggplot(aes(x=date,Winners_JT)) + scale_y_continuous(limits = c(-2,2))+geom_point()


#Find the mean of returns for Winners and Losers
winners_col<-subset(JT_portfolio, select=Winners_JT)
e<-na.omit(winners_col)
e<-tbl_df(e)
mean(e$Winners_JT)

e %>% ggplot(aes(x=date,Winners_JT)) + scale_y_continuous(limits = c(-2,2))+geom_point()

losers_col<-subset(JT_portfolio, select=Losers_JT)
m<-na.omit(losers_col)
mean(e$Losers_JT)
